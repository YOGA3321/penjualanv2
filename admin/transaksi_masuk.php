                            ...
                            <div class="d-flex gap-2">
                                ...
                                <?php elseif($pay_status == 'pending' && $row['metode_pembayaran'] == 'midtrans'): ?>
                                    <button class="btn btn-sm btn-primary flex-grow-1" onclick="cekStatusMidtrans(<?= $row['id'] ?>)">
                                        <i class="fas fa-sync"></i> Cek Status Bayar
                                    </button>
    ...
    function cekStatusMidtrans(id) {
        ...
        .then(d => {
            if(d.status === 'success') {
                ...
                // Jika masih pending, tawarkan bayar di kasir
                Swal.fire({
                    title: 'Status: Pending',
                    text: 'Pelanggan belum membayar di HP. Ingin bayar Tunai di Kasir?',
                    icon: 'info',
                    showCancelButton: true,
                    ...
                    if(res.isConfirmed) {
                        // Panggil API Detail dulu untuk dapat total harga
