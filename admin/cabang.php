...
if (isset($_GET['set_pusat'])) {
    ...
    // Set cabang terpilih sebagai pusat
...
if (isset($_GET['hapus'])) {
    ...
    if ($cek['is_pusat'] == 1) {
        $_SESSION['swal'] = [
            'icon' => 'error',
            'title' => 'Ditolak',
            'text' => 'Cabang Utama tidak boleh dihapus! Pindahkan dulu status pusatnya.'
        ];
    } else {
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0 fw-bold text-primary">
        <i class="fas fa-store-alt me-2"></i>Daftar Cabang
    </h4>
                ...
                <?php while ($row = $data->fetch_assoc()): ?>
                    <tr>
                        <td class="px-4 fw-bold">
                            ...
                            <?php if ($row['is_pusat']): ?>
                                <span class="badge bg-warning text-dark ms-2">
                                    <i class="fas fa-crown me-1"></i>PUSAT
                                </span>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="text-center">
                            ...
                            <?php else: ?>
                                <a href="cabang?set_pusat=<?= $row['id'] ?>"
                                   class="btn btn-outline-secondary btn-sm"
                                   onclick="return confirm('Jadikan cabang ini sebagai PUSAT UTAMA?')">
                                    Set Utama
                                </a>
                            <?php endif; ?>
                        </td>
                        ...
                        <td class="text-end px-4">
                            ...
                            <?php if (!$row['is_pusat']): ?>
                            <a href="cabang?hapus=<?= $row['id'] ?>"
                               class="btn btn-sm btn-danger"
                               onclick="return confirm('Hapus cabang ini? Data terkait mungkin ikut terhapus.')">
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text"
                                               name="nama_cabang"
                                               class="form-control"
                                               value="<?= htmlspecialchars($row['nama_cabang']) ?>"
                                               required>
                                    </div>
