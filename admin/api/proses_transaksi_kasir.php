...
if (!isset($_SESSION['user_id'])) {
    ob_clean(); echo json_encode(['status' => 'error', 'message' => 'Sesi habis. Login ulang.']); exit;
}

$json = file_get_contents('php://input');
$data = json_decode($json, true);

if (!$data) { ob_clean(); echo json_encode(['status' => 'error', 'message' => 'Data invalid']); exit; }
...
// =======================================================================
// 1. LOGIKA RESUME (Jika Transaksi Tertunda)
// =======================================================================
if ($metode == 'midtrans') {
...
// =======================================================================
// 2. VALIDASI: CEK KETERSEDIAAN MEJA (Antisipasi Tabrakan)
// =======================================================================
// Cek apakah meja sudah terisi oleh orang lain (yang bukan pending midtrans kita tadi)
$cek_meja = $koneksi->query("SELECT status FROM meja WHERE id = '$meja_id'")->fetch_assoc();
if ($cek_meja && $cek_meja['status'] == 'terisi') {
    ob_clean();
    echo json_encode(['status' => 'error', 'message' => 'Gagal! Meja ini baru saja terisi oleh pelanggan lain.']); 
    exit;
}

// =======================================================================
// 3. VALIDASI: CEK STOK REALTIME
// =======================================================================
foreach ($items as $item) {
    ...
    if ($cek_stok) {
        if ($cek_stok['stok'] < $qty_minta) {
            ...
            echo json_encode([
                'status' => 'error', 
                'message' => "Stok tidak cukup untuk menu: " . $cek_stok['nama_menu'] . ". (Sisa: " . $cek_stok['stok'] . ")"
            ]); 
            exit;
        }
    }
...
if ($stmt->execute()) {
    ...
    foreach ($items as $item) {
        ...
        // 2. [BARU] KURANGI STOK
        // Parameter: qty (jumlah berkurang), id (id menu)
