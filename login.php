...
<head>
    ...
    <style>
        .wrapper { height: auto !important; min-height: 440px; padding-bottom: 30px; }
        ...
        .divider { display: flex; align-items: center; text-align: center; color: rgba(255,255,255,0.6); margin: 20px 0; }
        .divider::before, .divider::after { content: ''; flex: 1; border-bottom: 1px solid rgba(255,255,255,0.3); }
        .divider:not(:empty)::before { margin-right: .5em; }
        .divider:not(:empty)::after { margin-left: .5em; }

        .btn-google {
            background-color: #fff; color: #333; display: flex; align-items: center; justify-content: center; 
            gap: 10px; padding: 12px; border-radius: 50px; text-decoration: none; font-weight: 600; 
            transition: all 0.3s ease; box-shadow: 0 4px 15px rgba(0,0,0,0.1); cursor: pointer;
        }
        .btn-google:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,0,0,0.2); }
        .btn-google img { width: 24px; height: 24px; }

        #ajax-login-form { display: none; animation: fadeIn 0.5s; }
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (togglePassword && passwordInput) {
            togglePassword.addEventListener('click', function() {
                ...
                if (type === 'text') { icon.classList.remove('fa-eye-slash'); icon.classList.add('fa-eye'); }
        ...
        if (loginForm) {
            loginForm.addEventListener('submit', function(event) {
                ...
                fetch('auth/proses_login.php', { method: 'POST', body: formData })
